...
const EditLectureComp = ({ cookie, id }) => {

    const [process, setprocess] = useState("Edit Lecture")
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                const { data } = await axios.get(`http://localhost:8000/api/admin/lecture/${id}/`, {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setprocess("Editing ...")
            const { data } = await axios.put(`http://localhost:8000/api/admin/lecture/${id}/edit/`, values, {
                withCredentials: true,
                headers: {
                    "X-CSRFTOKEN": cookie.csrftoken
                }
                    ...
                    <form action="" className='w-1/2' onSubmit={handleSubmit}>
                        ...
                        <div className='mb-3'>
                            ...
                            <select value={values.teacher} onChange={handleChange} type='text' name='teacher' className='w-full text-sm bg-[#caf0f8] border-none outline-none h-8 p-[-3px]' required>
                                <option value='' defaultValue>Select teacher</option>
                                {
                                    teachers?.map(item => {
                        ...
                        <div className='mt-3'>
                            <button disabled={process === "Edit Lecture" ? false : true} className='bg-[#2091F9] rounded-lg hover: py-[4px] font-medium capitalize text-white text-[16px] px-3 text-clrprimary10 transition-all ease-linear duration-300 w-36 disabled:cursor-not-allowed'>
                                {process}
                            </button>
                        </div>
                    </form>
